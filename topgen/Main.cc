...
class Frag
{
	public:
		...
		NodeContainer topNodes;		// topnode
	...
	private:
		...
		NetDevice()
		{
			...
			{
				oss << i << "-r0";
				NC.insert({ oss.str(), NodeContainer(leftNodes.Get(i),routerNodes.Get(0)) });
			...
			{
				oss << i << "-r1";
				NC.insert({ oss.str(), NodeContainer(topNodes.Get(i),routerNodes.Get(1)) });
				NDC.insert({ oss.str(), p2p.Install(NC[oss.str()]) });
				oss.str("");
				oss.clear();
			}
			...
			{
				oss << i << "-r" << nR - 1;
				NC.insert({ oss.str(), NodeContainer(rightNodes.Get(i),routerNodes.Get(nR - 1)) });
				NDC.insert({ oss.str(), p2p.Install(NC[oss.str()]) });
				oss.str("");
				oss.clear();
			}
			...
			{
				oss << 'r' << i << "-r" << i + 1;
				NC.insert({ oss.str(), NodeContainer(routerNodes.Get(i),routerNodes.Get(i + 1)) });
				oss.str("");
				oss.clear();
			}
